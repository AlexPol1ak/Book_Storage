...
class TextFormatter:
    ...
    @classmethod
    async def replace_whitespace(cls, text: str, new_symbol: str = '_') -> str:
        """
    ...
    @classmethod
    async def del_punctuation(cls, text: str) -> str:
    ...
    @classmethod
    async def translit_ru_en(cls, text: str, reverse: bool = False) -> str:
        """
    ...
    @classmethod
    async def name_formatter(cls, name: str,
        ...
        if punctuation_del:
            new_name = await cls.del_punctuation(new_name)

        if translit_text is not None:
            if translit_text == 'ru-en':
                new_name = await cls.translit_ru_en(new_name, reverse=True)
            elif translit_text == 'en-ru':
                new_name = await cls.translit_ru_en(new_name)
            else:
        ...
        if replace_space is not None:
            if isinstance(replace_space, str):
                new_name = await cls.replace_whitespace(new_name, new_symbol=replace_space)
            else:
                raise TypeError
